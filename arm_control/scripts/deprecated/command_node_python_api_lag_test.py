...
class CommandPublisherHebiAPI:
	...
	def __init__(self):
		...
		self.pubLagTest = rospy.Publisher('recv_time', Time, queue_size = 10)
		...
		# LAG TEST ARRAY - LOOK FOR 7 DISTINCT COMMANDS
		self.lagTestArray = [0, 0, 0, 0, 0, 0, 0]
		...
		rospy.Subscriber('joy', Joy, self.joystickUpdater) # 'joystick2'
	...
	def joystickHandler(self, data):
		...
		if not self.joyBlocked:
			...
			if self.mode == "rototiller":
				...
				if b[6] == 1: # BACK
					if self.lagTestArray[6] == 0:
						self.lagTestArray[6] = 1
						now = rospy.get_rostime()
						self.pubLagTest.publish(now)
					self.goHome(1)
					self.goHome(0)
				if b[7] == 1: # START
					self.goHome(1)
