...
class ObsAvoidance(object):
    def __init__(self):
        ...
        rospy.Subscriber("/utm_odom2", Odometry, self.odom_callback)
        rospy.Subscriber("/cmd_vel/managed", Twist, self.obs_avoidance)
    ...
    def obsMap(self, data):
        ...
        for point in pc2.read_points(data, skip_nans=True):
            ...
            if (
                pointz < self.heightThreshLow
                or pointz > self.heightThreshHigh
                or pointx > self.x
                or pointy >= self.y
                or pointx < 0
                or pointy < 0
    ...
    def obs_avoidance(self, data):
        P_gain = 0.5
        # extract the linear velocity and angular velocity
        linear = data.linear.x
        angular = data.angular.z
        cmd = Twist()
        density = bugAlgo.forward_section(self.map)
