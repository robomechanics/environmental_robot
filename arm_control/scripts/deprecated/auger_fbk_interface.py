...
class AugerFeedback:
	...
	def __init__(self):
		...
		rospy.init_node('AugerFeedback', anonymous=True)
		rospy.logwarn("initializing auger fbk interface...")
		...
		if rotoGroup == None:
			rospy.logerr("auger Hebi module not connected, failed to initialize auger fbk interface")
			return
		
		# set up publisher
		jsPub = rospy.Publisher('auger_hebi_fbk', JointState, queue_size=10)
		
		# get initial state values
		hebiFbk = rotoGroup.get_next_feedback(timeout_ms=1000)
		
		# complete initialization
		rospy.logwarn("auger fbk interface initialized")	
